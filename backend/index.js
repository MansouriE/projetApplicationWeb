...
app.post("/api/bids", async (req, res) => {
  try {
    const { article_id, usr_id, amount } = req.body;

    // 1. Validation basique
    if (!article_id || !usr_id || !amount) {
      return res
        .status(400)
        .json({ error: "Champs manquants (article_id, usr_id, amount)" });
    }
    ...
    // 3. Vérifier que l'article permet les enchères
    if (!article.bid) {
      return res
        .status(400)
    ...
    // 5. Vérifier le prix minimum
    const prixDepart = Number(article.bidPrixDeDepart ?? article.prix);

    // 6. Vérifier le bid actuel le plus élevé
    ...
    const { data: inserted, error: insErr } = await supabase
      .from("bids")
      .insert([{ article_id, usr_id, amount: montant }])
      .select("*")
      .single();
