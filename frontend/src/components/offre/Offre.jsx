...
export default function Offres() {
  ...
  useEffect(() => {
    ...
    const fetchOffers = async () => {
      try {
        ...
        const sentData = await fetchWithAuth(
  ...
  const handleAccept = async (offerId) => {
    try {
      ...
      // Use fetchWithAuth here as needed
    } catch (err) {
      console.error(err);
    }
  };
  ...
  if (!isLoggedIn)
    return (
      <p className="text-center mt-6">
        Veuillez vous connecter pour voir vos offres.
      </p>
    );
  if (loading)
    return <p className="text-center mt-6">Chargement des offres...</p>;
      ...
      <div className="w-full max-w-2xl p-6 space-y-8 bg-gray-100 rounded-xl shadow-md">
        ...
        <div>
          <h2 className="text-xl font-bold mb-4 text-center">
            Offres sur vos articles
          </h2>
          {receivedOffers.length === 0 ? (
            <p className="text-center text-gray-600">
              Aucune offre sur vos articles.
            </p>
          ) : (
              ...
              {receivedOffers.map((offer) => (
                ...
                >
                  <p className="text-sm md:text-base text-gray-700 mb-2 md:mb-0">
                    ...
                    pour l'article <strong>{offer.article_id}</strong>
                  </p>
              ...
              {sentOffers.map((offer) => (
                ...
                >
                  <p className="text-sm md:text-base text-gray-700 mb-2 md:mb-0">
                    ...
                    pour l'article <strong>{offer.article_id}</strong>{" "}
                    (propriétaire ID : {offer.owner_id})
                  </p>
                  <button
                    onClick={() => handleDecline(offer.id)}
